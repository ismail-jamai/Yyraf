...
const RegisterPage = () => {
  ...
  // State for code inputs and verification status
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error)
      alert('حدث خطأ أثناء التسجيل. المرجو المحاولة مرة أخرى.\n' + (error.response?.data?.message || ''))
    }
  }

  // ... (handlers for terms/privacy unchanged)
  ...
  const handleSendEmailCode = async () => {
    ...
    try {
      ...
      console.error(error)
      alert('فشل إرسال الكود. حاول مرة أخرى.')
    } finally {
  ...
  const handleVerifyEmail = async () => {
    try {
      await verificationService.verifyEmail(formData.email, emailCodeInput)
      setEmailVerified(true)
      alert('تم التحقق من البريد الإلكتروني بنجاح.')
    } catch (error) {
      setEmailVerified(false)
      alert('كود التحقق للبريد غير صحيح أو انتهت صلاحيته.')
    }
  }
  ...
  const handleSendPhoneCode = async () => {
    ...
    try {
      await verificationService.sendPhoneCode(formData.phone)
  ...
  const handleVerifyPhone = async () => {
    try {
      await verificationService.verifyPhone(formData.phone, phoneCodeInput)
      setPhoneVerified(true)
      alert('تم التحقق من رقم الهاتف بنجاح.')
    } catch (error) {
      setPhoneVerified(false)
      alert('كود التحقق للهاتف غير صحيح أو انتهت صلاحيته.')
    }
  }
                    ...
                    <div className="mb-8">
                      <div className="grid grid-cols-1 gap-4">
                        <button
                          ...
                          className={`p-4 rounded-lg border-2 transition-all ${formData.serviceType === 'dropshipping'
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="rounded-xl border p-4">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <button type="button" onClick={handleVerifyEmail} disabled={emailCodeInput.length !== 6 || emailVerified} className={`px-4 py-2 rounded-lg font-bold ${emailVerified ? 'bg-green-600 text-white' : 'bg-primary text-white hover:bg-primary-hover'}`}>تأكيد</button>
                        </div>
